App = {
    ...
    // 배포된 계약의 주소. (Ganache)
    contractAddress : '0x03a7b3362317dEb55f649197feF783b500019B44',   
    
    // 배포된 계약의 주인. (Ganache)
    ownerAddress : '0x9Db950812d37D540Dcb3FeD75Ae9dFF2F7f7d7F8',
    ...
    callAdoptPet : async function() {
        if (this.senderAddress != this.ownerAddress) {
            ...
            await this.myContract.methods.adoptPet(sent.id,sent.name,sent.contact).send({from:this.senderAddress, gas:300000, value:sent.cost})         
            // 현 페이지 refresh.
            location.reload();          
        } else {
...
$.getJSON('./js/pet-animals.json', function(data){          // 애완동물 정보를 불러온 이후 콜백함수 실행.
...
function submitModalAdopt(){
    ...
    console.log(`ID = ${id}는 ${name}에게 ${cost} Wei 비용으로 입양되었습니다! 연락처는 ${contact}입니다.`)
